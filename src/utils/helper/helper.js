...
export class helper{
    ...
    constructor()
    {
        ...
        const email  = (value,model) => this.isEmail(value,model)
        ...
        const time_to = {date:(unixTime,type)=>this.timeToDate(unixTime,type)}
        const response= () =>this.responseTime()
        
        // 链式操作 time 属性
        this.time     = { nature, to:time_to, response }
    ...
    */
     ...
     * @param {boolean} model [model=false] 严格模式
     * @return {boolean} result
     */
    isEmail(value, model = false)
    {
    ...
    formatBytes (bytes, decimals)
    {
        ...
        if (bytes === 0) result = '0 Bytes';
            ...
            result = (bytes / Math.pow(k, i)).toPrecision(dm) + ' ' + sizes[i];
        }
